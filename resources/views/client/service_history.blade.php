        ...
        <div class="container">
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <!-- Modal pour afficher les commentaires -->
           <!-- Modal pour afficher les commentaires -->
<div class="modal fade" id="viewCommentsModal-{{ $demande->id }}" tabindex="-1" aria-labelledby="viewCommentsModalLabel-{{ $demande->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="max-width: 800px;">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #007bff; color: white; padding: 10px 20px;">
                <h5 class="modal-title" id="viewCommentsModalLabel-{{ $demande->id }}">Commentaires sur {{ $demande->service->nom }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="color: white; filter: brightness(0) invert(1);"></button>
            </div>
            <div class="modal-body" style="padding: 20px; background-color: #f8f9fa;">
                @forelse ($demande->comments as $commentaire)
                <div class="d-flex {{ $commentaire->Userid ? 'justify-content-end' : 'justify-content-start' }}">
                    <div class="card {{ $commentaire->Userid ? 'bg-primary text-white' : 'bg-light' }}" style="max-width: 70%; margin-bottom: 10px; border-radius: 5px; overflow: hidden;">
                        <div class="card-footer text-muted" style="background-color: inherit; border-top: none; padding: 0.25rem 1.25rem; font-size: 0.8em;">
                            {{ $commentaire->Userid ? $commentaire->user->name : $client->name }}
                        </div>
